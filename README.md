...
* **Backend & Banco de Dados:** Python, Flask, Flask-SQLAlchemy, **Flask-CORS** (adicionado para permitir requisições cross-origin do frontend), SQLite.
...
3. Instale as dependências exigidas no projeto:
   ```bash
   pip install flask flask-sqlalchemy flask-cors google-generativeai pywhatkit streamlit openpyxl schedule plyer
   ```
   *(Nota: `flask-cors` foi adicionado para evitar bloqueios de segurança do navegador ao conectar o frontend com a API).*
...
* **Passo B: Acessar a Interface Web**
    ...
    *(Graças à configuração do **CORS aberto**, o frontend fará requisições assíncronas para a API sem sofrer bloqueios de origens diferentes no navegador).*
